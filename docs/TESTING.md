...
| Every reaction's stored answer balances; the all-1 state does not (except the seven trivially balanced ones, which the level pools skip); scalar multiples balance and simplify; the tier-3 hint always reaches a balanced lowest-terms equation; every species has a state symbol and a name; every level pool is big enough; the Challenge prompts name every species | `src/core-engine/tests/balancer-utils.test.ts`, `reactions.test.ts`, `chemical-utils.test.ts` (bracket parser) | — |
