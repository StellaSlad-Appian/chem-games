...
| [lewis-structures.md](./lewis-structures.md) | Share to Fill — build (pair the loners) + inspect (fix a classmate's drawing); Year 10, covalent only | Draft rev 2 (scope decided) | Medium | `lewis-structures` | `elements.ts` valenceElectrons; builds the shared `AtomCanvas` that Bond Builder reuses (separate games) |
