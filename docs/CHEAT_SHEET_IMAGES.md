...
| # | Key | Size | What it should show |
|---|---|---|---|
| 1 | `relative-formula-mass/01-carbon-hydrogen-balance` | 360×320 | Under *What "relative" actually means*: a two-pan balance, level, with one carbon atom (a ring marked C) on the left pan and twelve hydrogen atoms (smaller rings marked H, in rows of 5, 4 and 3) on the right. Under the pans, *1 carbon atom* and *12 hydrogen atoms*; under both, the glossary's *not to scale*. Carbon's radius is 2.1 × hydrogen's (their covalent radii), so about 4.4 times the area — larger, and nowhere near 12 ×, because size is not mass. The run checks that 12 × 1.008 is within 1% of 12.011, that the class-table masses (H 1, C 12) balance exactly, and that carbon is drawn at under half of 12 times hydrogen's area. |
