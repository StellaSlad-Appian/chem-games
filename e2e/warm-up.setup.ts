...
setup('compile every route', async ({ page }) => {
  ...
  const routes = [
    ...
    // specs fail in de, fr, es and it while passing in en: six workers hit a
    // brand-new route at once, one of them paid the compile and the others
    // were served a document whose <html> had no `lang` yet. Warming the two
    // dynamic segments warms the route, not the entry — any id compiles the
    // same page.
    '/explore',
    '/explore/scientist',
    '/explore/archive',
