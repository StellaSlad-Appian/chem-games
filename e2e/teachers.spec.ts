...
// site-wide feedback button — the page's one call to action that is not the
// collaborator sign-up — is really there, and that nothing errors in the
// console. The sign-up form itself moved to its own page and its browser
// coverage moved with it, to e2e/teachers-collaborate.spec.ts.
//
...
test.describe('For Teachers', () => {
  ...
  test('offers the site-wide feedback button, and a link to the sign-up page rather than the form itself', async ({
    page,
  }) => {
    await page.goto(path('/teachers'));

    await expect(page.getByRole('button', { name: en.feedback.openA11y })).toBeVisible();
    // The sign-up form moved to its own page (docs/COLLABORATORS.md § 4); this
    // page carries a pitch and a link onward, not the form itself, so a
    // second form is no longer even a risk here.
  ...
  test('links out to privacy, explore, the games and the cheat sheets', async ({ page }) => {
    await page.goto(path('/teachers'));

    const main = page.locator('main');
    await expect(
      main.getByRole('link', { name: new RegExp(enTeachers.exploreLinkLabel, 'i') })
