...
export const ru = {
  heading: 'Учителям',
  intro:
    'Что такое Games in Chemistry, что на сайте есть и как вы можете помочь его сформировать. Все остальные страницы написаны для школьников, которые здесь играют; эта — для вас.',

  betaHeading: 'Сайт в бета-версии',
  betaBody:
    'Games in Chemistry ещё строится. Игры меняются, появляются новые, а формулировка подсказки или шпаргалки через месяц может быть другой. Всё, что здесь есть, работает, и всё бесплатно — но пройдите игру сами, прежде чем давать её классу.',
  ...
  sheetsHeading: 'Шпаргалки',
  sheetsIntro:
    'Пятнадцать шпаргалок собирают справочный материал, на который опираются игры. Каждая умещается на одну страницу, читается с проектора и печатается:',

  exploreHeading: 'Мир открытий',
  exploreBody1:
    'Каждый понедельник — новое вещество и новый химик, и о каждом — несколько коротких разделов: где это встречается в жизни, как это работает и почему человек за этим важен.',
  exploreBody2:
    'Когда неделя проходит, у её материала остаётся постоянная ссылка, и архив хранит их все — урок может сослаться на вещество или химика месячной давности так же легко, как на материал этой недели. Текущая неделя — на {link}.',
  exploreLinkLabel: 'странице «Мир открытий»',
  ...
  languagesBody3:
    'На шпаргалке на любом языке, кроме английского, не хватает двух вещей. Внешних ссылок на ней нет, потому что все сайты, на которые мы ссылаемся, англоязычные, и никакая программа на ней не названа, потому что единственная, что у нас есть, — австралийская Victorian Curriculum. Показывать ученику материал, которым он не сможет воспользоваться, хуже, чем не показывать ничего, поэтому и то и другое мы не переводим, а убираем. Местные соответствия — сопоставление с программой каждой страны и ссылки, которые ученик сможет прочитать, — мы пока не подбирали.',
  ...
  privacyBody2:
    'Чтобы играть, аккаунт не нужен. О том, кто играет без аккаунта, на сайте не остаётся записей: браузер хранит у себя настройки звука, темы и доступности, а также отметку о том, была ли уже показана инструкция к каждой игре, а с запросами передаётся одна cookie — в ней только выбранный язык.',
  privacyBody3:
  ...
  collaborateHero: 'Помогите сформировать то, что будет дальше. Взамен — версии 1.0 и 2.0 бесплатно.',
  collaborateWhat:
  ...
  collaborateCta: 'Чтобы отозваться, заполните форму на отдельной странице: {link}.',
  collaborateCtaLinkLabel: 'Запись',

  collaborateMetaTitle: 'Записаться в соавторы | Учителям | Games in Chemistry',
  collaborateMetaDescription:
    'Расскажите, как прошла игра, или предложите новую, и получите взамен бесплатный доступ к версиям 1.0 и 2.0.',
  backToTeachers: 'Назад к разделу «Учителям»',
  collaborateHow:
    'Чтобы отозваться, заполните форму ниже. По-настоящему нужен только адрес электронной почты; остальные поля стоят там, чтобы до ответа было понятно хоть что-то про ваш класс, и любое из них можно пропустить.',
  collaborateReply:
  ...
  collaborateRecords:
    'Почему форма, а не кнопка отзыва, на которую эта страница ссылалась раньше: отзывы попадают в почтовый ящик, а ящик — не список. Без надёжного места, где хранить адрес, обещание про 1.0 и 2.0 выполнить невозможно. Вот и вся причина, по которой здесь спрашивают адрес.',

  // Форма записи. Страница передаёт её строки в `CollaboratorForm` пропсами;
  // сам компонент этот каталог никогда не импортирует — см.
  // docs/COLLABORATORS.md § 4.
  formHeading: 'Записаться в соавторы',
  formIntro:
    'Обязателен здесь только адрес электронной почты. Остальное можно оставить пустым: другие поля нужны мне, и ни одно из них не является условием.',
  formUse:
    'Адрес нужен, чтобы писать вам об играх и дать доступ к версиям 1.0 и 2.0. Больше ни для чего, и никому он не передаётся. Письма с подтверждением тоже не будет: следующее, что придёт от меня, — ответ, написанный вручную.',
  formDelete:
    'Свои данные можно удалить когда угодно, и для этого не нужны ни аккаунт, ни форма: напишите на {email}, что хотите выйти из списка, и запись удалят.',
  formOptional: 'необязательно',

  formEmailLabel: 'Адрес электронной почты',
  formEmailHelp: 'Тот адрес, на который вам удобнее получать ответ.',
  formNameLabel: 'Ваше имя',
  formSchoolLabel: 'Школа',
  formCountryLabel: 'Страна',
  formYearLevelsLabel: 'Классы, в которых вы преподаёте',
  formYearLevelsHelp: 'Так, как их называет ваша школа, — например, 8 и 9 класс.',
  formSubjectsLabel: 'Предметы, которые вы ведёте',
  formSubjectsHelp: 'Например, химия и естествознание в средних классах.',
  formMessageLabel: 'Чем хотели бы помочь',
  formMessageHelp:
    'Как игра прошла в классе, идея игры, которой пока нет, или и то и другое. Хватит одного предложения.',

  formSubmit: 'Записаться',
  formSubmitting: 'Отправляем…',
  formSuccessTitle: 'Спасибо — вы в списке.',
  formSuccessBody:
    'Пока больше ничего не происходит: письмо с подтверждением не идёт, и адрес не используется ни для чего, кроме ответа вам.',
  formGenericError:
    'Что-то пошло не так, и запись не сохранилась. Попробуйте ещё раз через минуту.',
