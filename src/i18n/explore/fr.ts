...
export const EXPLORE_OVERLAY_FR = {
  ...
  scientists: {
    ...
    'marie-maynard-daly': {
      work: 'Marie Maynard Daly a travaillé sur deux problèmes très différents. À l’Institut Rockefeller, à partir de 1948, avec Alfred Mirsky, elle a étudié la chimie du noyau de la cellule. C’est-à-dire les protéines histones autour desquelles l’ADN s’enroule, et ce dont sont faits les acides nucléiques. À partir de 1955, avec le médecin Quentin Deming, elle s’est tournée vers les artères. Leurs expériences, menées surtout sur des rats hypertendus, ont été parmi les premières à relier trois choses qu’on avait étudiées séparément : la pression sanguine élevée, le cholestérol et le rétrécissement des artères.',
      legacy:
    ...
    'dan-shechtman': {
      work: 'Les cristaux se répètent. Ce n’était pas une supposition, c’était la définition : les atomes occupent un motif qui se recopie indéfiniment dans toutes les directions. Un tel motif ne peut pas avoir de symétrie d’ordre cinq ou dix. On ne peut pas carreler un sol avec des pentagones sans laisser de trous. Le 8 avril 1982, Dan Shechtman était détaché du Technion de Haïfa dans un laboratoire public américain. Il a envoyé des électrons à travers un alliage aluminium–manganèse refroidi très vite, et il a obtenu une figure de diffraction de symétrie d’ordre dix. Son cahier porte, pour cet échantillon, la mention « 10 fold ??? ». Les atomes étaient parfaitement ordonnés – on pouvait dire où devait se placer le suivant – mais l’arrangement ne se répétait jamais.',
      legacy:
