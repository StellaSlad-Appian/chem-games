...
//   * "Unpaired electron" is the *only* name the game gives an outer electron
//     without a partner. There is no second, playful tier: the game's invented
//     nickname — English "loner", and each locale's equivalent — was dropped on
//     2026-09-19, so every locale now uses its own textbook term everywhere,
//     including on the canvas. German *ungepaartes Elektron*, French *électron
//     célibataire*, Spanish *electrón desapareado*, Italian *elettrone
//     spaiato*, Russian *неспаренный электрон*. The rejected nicknames are
//     recorded in docs/i18n/glossary-<locale>.md so nobody re-proposes them.
...
//     must start and end with a letter in any alphabet — see
//     docs/i18n/glossary-de.md and GlossaryTerm.tsx.
...
export const LEWIS_STRUCTURES_MESSAGES = {
  ...
  instructions: {
    ...
    intro:
      'Every atom brings its outer electrons as dots. A dot on its own is an unpaired electron — it wants a partner. Two unpaired electrons from two different atoms make a shared pair, which is a bond.',
    bullets: [
    ...
    keyboard: [
      ['Tab', 'selects an atom'],
      ['← →', 'picks one of its unpaired electrons'],
  ...
  guided: {
    ...
    h2: [
      'Two hydrogen atoms. Each has 1 outer electron — an unpaired electron. Drag one onto the other.',
      "They now share a pair. Count around each H: 2. Hydrogen is full at 2 — that's a single bond, H–H.",
    ],
    h2oStep1:
      'Oxygen has 6 outer electrons: two pairs (they stay put) and two unpaired electrons (they pulse).',
    h2oStep2: 'Pair one unpaired electron on the oxygen with the one on a hydrogen.',
    h2oStep2After: 'Oxygen now has 7 around it — one more to go.',
    h2oStep3: 'Pair the other unpaired electron on the oxygen with the other hydrogen.',
    h2oStep4:
  ...
  coach: {
    label: 'Coach',
    loners: {
      one: '{atom} still has {count} unpaired electron. It pairs with an unpaired electron on another atom.',
      other: '{atom} still has {count} unpaired electrons. They pair with unpaired electrons on another atom.',
    },
    needsMore:
      '{atom} has {count} of 8. It needs another shared pair — which atom still has an unpaired electron?',
    shareAgain:
      '{atom1} and {atom2} both still have an unpaired electron. They can share again — that makes a double bond.',
    complete:
      'Every atom is full and no unpaired electrons are left. This is {name}: {bonds}, {lonePairs}.',
    sameGroup:
    ...
    central: 'The atom with the most unpaired electrons usually goes in the middle.',
    deadEnd:
      '{atom} has {count} of 8, but no other atom has an unpaired electron left to share. Tap a shared pair to undo it, then try a different partner.',
    isomer:
  ...
  hint: {
    ...
    tier3: 'Pair the unpaired electron on {atom1} with the one on {atom2}.',
    ...
    inspectTier3Repair: 'Pair up the unpaired electrons until every atom is full again.',
  ...
  error: {
    label: 'Not that move',
    atomFull:
      "{atom} already has 8 — it can't share any more. Try an atom that still has an unpaired electron.",
    ...
    pairedDot:
      'That dot is already part of a pair. Only unpaired electrons (the pulsing ones) can be shared.',
  },
  ...
  inspect: {
    ...
    explainNeedsDouble:
      '{atom1} and {atom2} each still have an unpaired electron — they need to share twice.',
    explainLeftover: '{atom} has an unpaired electron left over — an extra electron was drawn.',
    ...
    repair: 'Now fix it: pair up the unpaired electrons until every atom is full.',
  ...
  overlay: {
    ...
    levelUpSubtitle: 'No unpaired electrons left',
  ...
  glossary: {
    ...
    unpairedElectron: {
      ...
      definition: 'an outer electron without a partner; only these can be shared',
  ...
  ui: {
    ...
    atomLonerA11y: '{element}, unpaired electron {index} of {total}',
    atomLonePairA11y: '{element}, lone pair {index} of {total}',
    atomOrdinal: '{element} {ordinal}',
    unpairedLabel: 'unpaired electron',
    atomFull: 'full',
    atomSelectedA11y:
      '{element} unpaired electron selected. Now choose an unpaired electron on another atom.',
