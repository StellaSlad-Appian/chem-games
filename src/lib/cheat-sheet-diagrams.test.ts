...
/**
 * The fixed custom properties: the one `:root` rule that names no theme. A
 * token here has the same value in both themes (the pH indicator colours).
 */
function fixedProperties(): Map<string, string> {
  const properties = new Map<string, string>();
  const withoutComments = globalsCss.replace(/\/\*[\s\S]*?\*\//g, '');
  for (const [, selector, body] of withoutComments.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
    if (selector.trim() !== ':root') continue;
    for (const [, name, value] of body.matchAll(/(--[\w-]+)\s*:\s*([^;]+);/g)) {
      properties.set(name, value.trim());
    }
  }
...
describe('universal-indicator colours', () => {
  ...
  const fixed = fixedProperties();
  const themedDark = themeProperties('dark');
  const light = themeProperties('light');
  const dark = new Map([...fixed, ...themedDark]);

  it('are fixed: one value per pH, set once in :root and not themed', () => {
    for (const token of steps) {
      expect(fixed.get(token), `${token} is not set`).toMatch(/^#[0-9a-f]{6}$/i);
      expect(themedDark.has(token), `${token} is overridden in the dark theme`).toBe(false);
